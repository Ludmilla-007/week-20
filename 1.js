...

{
    ...
    for (let marvel of marvels){
        marvelsContent +=`<div class ='marvel'>
        <h2>${marvel.title}</h2>
        <div>Вселенная:${marvel.universe}</div>
        <div>Альтер эго:  ${marvel.ego}</div>
        <div>Род деятельности:${marvel.type_of_activity}</div>
        <div>Друзья:${marvel.friends}</div>
        <div>Суперсилы:${marvel.superpower}</div>
        <img src=${marvel.photo}>
        <div>Подробнее:${marvel.more}</div>
        <div><select>
    ...
    document.getElementById("marvelsContainer")
    .innerHTML = marvelsContent;
